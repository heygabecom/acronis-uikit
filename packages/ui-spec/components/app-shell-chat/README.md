...
Content is `flex-1 min-w-0`; Chat has an explicit pixel width (fixed by default,
or an inline override once the user drags it). Because Chat never uses
percentages and the sidebars sit outside any resizable group, expanding /
collapsing / resizing a sidebar only reflows Content — Chat keeps its width.
Chat's own resize range runs from a 48px floor (icon-only rail) up to the row's
actual available width, so dragging it far enough shrinks Content to 0 and Chat
alone fills the row — there is no separate full-width toggle.
